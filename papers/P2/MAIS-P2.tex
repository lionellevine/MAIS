...
\title{A refutation and a repair of Critch's PBL theorem, with an application to program cooperation}
\author{GPT-5.6 Sol}
\date{July 2026}
...
\paragraph{\hyperref[sec:pbl-refutation]{Section~\ref*{sec:pbl-refutation}: the PBL refutation.}}
...
\begin{proposition}[Refutation of \CritchPBLRef]\label{prop:failure}
...
The repair promotes the three missing comparisons to explicit hypotheses.  Assume that each graph formula below is provably total and single-valued in $S$, so that it may be treated as a function symbol in a definitional extension.  For represented functions $u,v:\N\to\N$, write
\[
  u\leSstar v
\]
...
Let $p[k]\in L_S(1)$, and let $f,g,h:\N\to\N$ be represented computable functions with provably total and single-valued graphs.  Suppose:
...
The substantive issue is that the proof turns external comparisons into uniform theorems when it enlarges the proof bounds in the cooperation argument; it then invokes the faulty PBL theorem.  (As usual, we assume $S$ is sound for the terminating computations under discussion.)  The diagonal construction below shows that this is not merely a missing proof step: on the literal, implementation-sensitive reading, the semantic conclusion is false.

\subsection{The published robust-cooperation theorem is false as stated}

\begin{proposition}[Refutation of \CritchCooperationRef]\label{prop:coopfailure}
...
\]
Taking $B_k=A_k$ refutes \CritchCooperationRef.
\end{proposition}
...
The parameterized recursion theorem and inert source-code padding permit a simultaneous construction of the agents and their common budget subroutine.  For each $k$, let $A_k$ be the FairBot of~\cite{Critch}, using a common subroutine $G$.  Among the infinitely many padded sources for this program, choose one for which the concrete proof-search target
\[
  P_k:=\bigl(A_k(A_k)=\Coop\bigr)
\]
has more than $k$ characters in the fixed proof syntax of~\cite{Critch}.  This is possible because the padded sources have distinct G\"odel codes and therefore distinct numeral terms.  Over a finite proof alphabet, their lengths are unbounded, so an appropriate source can be found effectively.  Make these choices successively so that the exact source lengths
\[
  L_k:=\Len(A_k)
\]
...
If $G(L_k)=0$, the actual self-play proof bound is $k$.  But every proof of $P_k$ must contain its conclusion, directly or in the definition of an abbreviation, and $P_k$ has more than $k$ characters.  Hence no proof of $P_k$ has length at most $k$, and again $A_k$ defects.  Thus $P_k$ is false in either branch.
...
The repair changes the formal generality of both results.  The published statements are extensional: they purport to depend only on the standard functions $f$ and $G$ and their asymptotic growth.  The repaired statements are intensional as well: they depend on what the chosen representations and implementations allow $S$ to prove about the relevant resource bounds.  Propositions~\ref{prop:failure} and~\ref{prop:coopfailure} show that representability or provable totality alone cannot erase this distinction.
