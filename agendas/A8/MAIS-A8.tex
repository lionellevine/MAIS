...
\hypersetup{
  ...
  pdfsubject={Research agenda supplement to MAIS-O8 of Math for AI Safety}
}
\begin{document}
\begin{abstract}
...
\begin{problem}[\Oid{8}: The selection map]\label{prob:selectionmap}
...
\begin{conjecture}[\Oid{82}: The proxy wins at $\varepsilon=0$]\label{conj:proxywins}
...
\begin{question}[\Oid{83}: Is there any coin-lover in the standard family?]\label{q:coinlover}
...
\begin{problem}[\Oid{8}, $\varepsilon>0$: Diversity at the level of networks]\label{prob:diversitynets}
...
\begin{problem}[\Oid{84}: The crossover time, or: why $2\%$?]\label{prob:crossover}
...
\begin{problem}[\Oid{85}: Which representations misgeneralize?]\label{prob:encodings}
...
\begin{question}[\Oid{86}: The Bayesian-sampler test, in a box]\label{q:bayes}
...
\begin{question}[\Oid{87}: Exploration starvation]\label{q:starvation}
...
\begin{problem}[\Oid{88}: The misgeneralization curve]\label{prob:curve}
...
\begin{problem}[\Oid{89}: The boundary residual]\label{prob:boundary}
...
\begin{problem}[\Oid{84}: The crossover asymptotics]
...
\begin{problem}[\Oid{90}: Kernel-regime selection, end to end]\label{prob:ntkstarter}
...
\begin{problem}[\Oid{91}: The atlas]
...
\bibitem{mei2018} Mei, S., Montanari, A., and Nguyen, P.-M. (2018). A mean field view of the landscape of two-layer neural networks. \emph{Proceedings of the National Academy of Sciences} 115(33), E7665--E7671. \arxiv{1804.06561}.
