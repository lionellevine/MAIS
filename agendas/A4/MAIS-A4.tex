...
\hypersetup{
  ...
  pdfsubject={Research agenda supplement to MAIS-O4 of Math for AI Safety}
}
\begin{document}
\begin{abstract}
...
\begin{center}
\small\emph{Research agenda MAIS-A4 \(\cdot\) Claude Fable 5, audited by GPT 5.6 Sol\\
July 12, 2026 \(\cdot\) Status: draft}
\end{center}

\noindent This is a supplement to Open Problem~\texttt{MAIS-O4} of Lionel Levine's \emph{Math for AI Safety: An Invitation for Mathematicians}~\cite{mais}.

\begin{quote}
\textbf{Open Problem \texttt{MAIS-O4} (Training for interpretability).}
...
\noindent Landing page, with provenance and audit record: \url{https://github.com/lionellevine/MAIS/tree/main/agendas/A4}.
...
\begin{problem}[\Oid{4}: Coherence-constrained frontier]\label{prob:frontier}
...
\begin{problem}[\Oid{44}: Penalizing the sum to control the max]\label{prob:summax}
...
\begin{problem}[\Oid{45}: Recovery transfer: ``more identifiable atoms'']\label{prob:transfer}
...
\begin{question}[\Oid{46}: Does coherence rank recoverability?]\label{q:goodhart}
...
\begin{problem}[\Oid{47}: Single-orbit training]\label{prob:orbit}
...
\begin{problem}[\Oid{48}: Monosemanticity frontier]\label{prob:mono}
...
\begin{problem}[\Oid{49}: The wiring frontier]\label{prob:wsparse}
...
\begin{question}[\Oid{50}: What does gradient flow select?]\label{q:dynamics}
...
\begin{example}[\Oid{51}: The regularized phase diagram of the smallest model]\label{ex:21}
