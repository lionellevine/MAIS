...
\hypersetup{
  ...
  pdfsubject={Research agenda supplement to MAIS-O3 of Math for AI Safety}
}
\begin{document}
\begin{abstract}
...
\noindent This is a supplement to Open Problem~\texttt{MAIS-O3} of Lionel Levine's \emph{Math for AI Safety: An Invitation for Mathematicians}~\cite{mais}.

\begin{quote}
\textbf{Open Problem \texttt{MAIS-O3} (The geometry and identifiability of superposition).}
...
\noindent Landing page, with provenance and audit record: \url{https://github.com/lionellevine/MAIS/tree/main/agendas/A3}.
...
\begin{problem}[\Oid{3}: Identifiability of the $\ell^1$ dictionary estimator]\label{prob:main}
...
\begin{conjecture}[\Oid{36}: Recovery under independent supports]\label{conj:independent}
...
\begin{problem}[\Oid{37}: Two smeared features: recover, merge, or split]\label{prob:split}
...
\begin{problem}[\Oid{39}: Does the encoder change the answer?]\label{prob:amortization}
...
\begin{conjecture}[\Oid{40}: Pentagon]\label{conj:pentagon}
...
\begin{problem}[\Oid{41}: Starter: the exact two-feature phase diagram]\label{prob:starter-phase}
...
\begin{problem}[\Oid{42}: Starter: the one-active-feature pentagon]\label{prob:starter-pentagon}
...
\begin{problem}[\Oid{43}: Starter: measure the phase diagram before proving it]\label{prob:starter-numerics}
