...
\hypersetup{
  ...
  pdfsubject={Research agenda supplement to MAIS-O7 of Math for AI Safety}
}
...
\noindent This is a supplement to Open Problem~\texttt{MAIS-O7} of Lionel Levine's \emph{Math for AI Safety: An Invitation for Mathematicians}~\cite{mais}.

\begin{quote}
\textbf{Open Problem \texttt{MAIS-O7} (Opposing staircases).}
...
\noindent Landing page, with provenance and audit record: \url{https://github.com/lionellevine/MAIS/tree/main/agendas/A7}.
...
\begin{question}[\Oid{72}]\label{q:strat}
...
\begin{problem}[\Oid{73}: singular Eyring--Kramers]\label{prob:kramers}
...
\begin{conjecture}[\Oid{74}]\label{conj:kramers}
...
\begin{problem}[\Oid{75}: two-stratum entropic selection]\label{prob:entropic}
On $\T^2$, let
\[
...
\begin{problem}[\Oid{76}: effective diffusion beyond the smooth part]\label{prob:katzenberger}
...
\begin{problem}[\Oid{77}: $\lambda$-stratification of the fiber and its saddles]\label{prob:fiber}
...
\begin{conjecture}[\Oid{7}: opposing staircases]\label{conj:opposing}
...
\begin{problem}[\Oid{78}: time versus sample size]\label{prob:dictionary}
...
\begin{question}[\Oid{79}]\label{q:effective-loss}
...
\begin{problem}[\Oid{80}]\label{prob:leap}
...
\begin{problem}[\Oid{81}]\label{prob:saddle-free-energy}
