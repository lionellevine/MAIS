...
\noindent This is a supplement to Open Problem~\texttt{MAIS-O6} of Lionel Levine's \emph{Math for AI Safety: An Invitation for Mathematicians}~\cite{mais}.

\begin{quote}
\textbf{Open Problem \texttt{MAIS-O6} (Does geometric simplicity force a legible mechanism?)}
...
\noindent Landing page, with provenance and audit record: \url{https://github.com/lionellevine/MAIS/tree/main/agendas/A6}.
...
\begin{question}[\Oid{62}]\label{q:width}
...
\begin{problem}[\Oid{63}: Learning coefficients of modular addition]\label{prob:modadd}
...
\begin{conjecture}[\Oid{64}: The marginal cost of width]\label{conj:increment}
...
\begin{conjecture}[\Oid{6}: Fourier structure of the most degenerate solutions]\label{conj:fourier}
...
\begin{problem}[\Oid{65}: Cross-entropy version]\label{prob:xent}
...
\begin{conjecture}[\Oid{66}: Generic constancy]\label{conj:generic}
...
\begin{problem}[\Oid{67}]\label{prob:attn-teacher}
...
\begin{problem}[\Oid{68}: Linear attention]\label{prob:linear-attn}
...
\begin{problem}[\Oid{69}: When does Gaussian localization localize?]\label{prob:estimator}
...
\begin{problem}[\Oid{70}: Local coefficients on the template]\label{prob:calibration}
...
\begin{question}[\Oid{71}: Continuous-input ReLU learning]\label{q:relu}
Fix $r,H\ge1$ and
\[
