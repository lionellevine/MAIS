...
\newcommand{\Oid}[1]{\href{https://github.com/lionellevine/MAIS/blob/main/open-problems/MAIS-O#1.md}{\texttt{MAIS-O#1}}}
...
\noindent This is a supplement to Open Problem~\texttt{MAIS-O5} of Lionel Levine's \emph{Math for AI Safety: An Invitation for Mathematicians}~\cite{mais}.

\begin{quote}
\textbf{Open Problem \texttt{MAIS-O5} (Representation theory of learned circuits).}
...
\noindent Landing page, with provenance and audit record: \url{https://github.com/lionellevine/MAIS/tree/main/agendas/A5}.
...
The headline problem is Open Problem~\texttt{MAIS-O5} in measurable form. Its weight decay $\lambda > 0$ is not incidental: that is the regime of the survey statement and of every seed table in Section~\ref{sec:phenomenon}. The survey also singles out the first case with genuine competition: $C_5$, quadratic activation, width two, and no decay. Problem~\ref{start:c5} makes that foothold explicit.

\begin{problem}[\Oid{5}: The selection law]\label{prob:law}
...
\begin{problem}[\Oid{52}: Sparse or dense]\label{prob:sparse}
...
\begin{conjecture}[\Oid{53}: Law of large numbers for multiplicities]\label{conj:lln}
...
\begin{question}[\Oid{54}: Selection probabilities]\label{q:dsquared}
...
The cyclic foothold $C_5$ has the advantage of matching Open Problem~\texttt{MAIS-O5} exactly. For a first \emph{nonabelian} laboratory, take $G=S_3$. Its representation theory still fits on one line: $\Rcl(S_3) = \{\mathrm{sgn}, \mathrm{std}\}$, with $d_{\mathrm{sgn}} = 1$, $d_{\mathrm{std}} = 2$, and only $\mathrm{std}$ faithful.

\begin{problem}[\Oid{55}: $S_3$ with the architecture fixed in advance]\label{prob:s3}
...
\begin{conjecture}[\Oid{56}]\label{conj:s3}
...
\begin{problem}[\Oid{57}: Non-identifiability at width 128]\label{prob:nonident}
...
\begin{question}[\Oid{58}: Exchangeability beyond symmetry]\label{q:exchangeable}
...
\begin{problem}[\Oid{59}: The smallest case with competition]\label{start:c5}
...
\begin{problem}[\Oid{60}: One rectifier neuron]\label{start:relu}
...
\medskip
\noindent\textbf{Resolved, in the negative} (Gautam Neelakantan Memana, August 2026). For every odd prime $p \ge 5$ there is an open set of active initializations---hence a positive-probability event conditional on activity---from which the flow extinguishes the sole active gate in finite time: the weights freeze at a centered point mass whose Fourier energy is $2/(p-1)$ in every nonzero frequency class, so the normalized limit exists but is not single-frequency pure for any $\delta < 1 - 2/(p-1)$. A strengthened analysis by GPT-5.6 Sol exhibits a second open set on which the neuron stays active forever, memorizes a single table entry, and diverges in norm with the same flat limiting spectrum, and shows both failures are independent of the choice of Clarke trajectory. The resolution record, with proofs, is linked from the problem's page in the repository. What survives is the quantitative question, Problem~\ref{start:outcome} below.

\begin{problem}[\Oid{61}: A pilot measurement of the law]\label{start:measure}
...
\begin{problem}[\Oid{92}: The outcome law of one rectifier neuron]\label{start:outcome}
In the setting of Problem~\ref{start:relu}, say that a Clarke trajectory \emph{dies} if it reaches a completely inactive state in finite time and is constant thereafter; \emph{aligns} if $\theta(t)/\|\theta(t)\|$ converges to a limit that is $(\delta,[\rho_\zeta])$-pure for every $\delta>0$ and some frequency $\zeta$; and \emph{memorizes} if the normalized limit exists but is not pure. The resolution of Problem~\ref{start:relu} shows that death and memorization each occur with positive probability conditional on initial activity. Determine the outcome law:
(1) does alignment occur with positive probability?
(2) determine, or bound as $p \to \infty$, the conditional probabilities of death, memorization, and alignment;
(3) how does the law depend on the initialization scale $\tau$?
(4) for an everywhere-positive activation---softplus $\sigma_\beta(x) = \beta^{-1}\log(1+e^{\beta x})$, where no gate can die and the known counterexample mechanisms are unavailable---does the alignment conclusion of Problem~\ref{start:relu} hold almost surely?
Part (1) alone would be substantial progress; the known counterexamples occupy constructed corners of initialization space and say nothing about typical behavior.
\end{problem}
...
Open Problem~\texttt{MAIS-O5} asks for the distribution of representations visible in the outputs. The key set $K_\varepsilon$ makes that question precise. At zero weight decay, however, cross-entropy may have no finite minimizer: scaling a strictly separating ray sends the loss toward zero. This is why the $C_5$ foothold in Problem~\ref{start:c5} asks about normalized directions, as in the margin framework of \cite{wei2019,lyu2020,morwani2024}, rather than finite minima.
...
2026. \url{https://github.com/lionellevine/MAIS/tree/main/papers/P1}.
