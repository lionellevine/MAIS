...
\hypersetup{
  ...
  pdfsubject={Research agenda supplement to MAIS-O1 of Math for AI Safety}
}
...
\noindent This is a supplement to Open Problem~\texttt{MAIS-O1} of Lionel Levine's \emph{Math for AI Safety: An Invitation for Mathematicians}~\cite{mais}.

\begin{quote}
\textbf{Open Problem \texttt{MAIS-O1} (Quantitative bounded L\"ob).}
...
\noindent Landing page, with provenance and audit record: \url{https://github.com/lionellevine/MAIS/tree/main/agendas/A1}.
...
\label{rem:pbl-audit}
The unrestricted graph-based version is false under the representation conventions used here. A budget can equal a rapidly growing function at every standard input while its graph branches to zero if a bounded proof of contradiction exists; $S$ then proves the reflection premise for $p(k)=\bot$. Provable totality and single-valuedness alone do not exclude this construction. The certificate hypotheses above rule out the hidden zero branch. The full construction, together with the parallel refutation and repair of the robust-cooperation theorem, is the companion note \cite{sol2026}.
\end{remark}
...
\begin{problem}[\Oid{1}: Determine the overhead]
...
\begin{conjecture}[\Oid{10}: Polynomial overhead]
\label{conj:poly}
...
\begin{question}[\Oid{11}: Lower bounds]
\label{q:lower}
\hspace{1em}
\begin{enumerate}
...
\begin{problem}[\Oid{12}: System constants]
...
\begin{conjecture}[\Oid{13}: Comparison of budget terms]
...
\begin{question}[\Oid{14}: The L\"ob window]
\label{q:window}
...
\begin{problem}[\Oid{15}: Effective parametric bounded L\"ob]
...
\begin{problem}[\Oid{16}: The FairBot threshold]
\label{prob:fairbot}
...
Critch, Dennis, and Russell \cite{cdr2022} observe that proof \emph{search} need not be the bottleneck: an agent can try the known L\"obian proof first rather than enumerate every shorter string. This makes fast verification plausible, but their paper does not implement or benchmark the proposal. Measuring $\hat{k}^{*}$ in a real proof assistant remains the experiment; see Starter~\ref{start:machine}.
...
\begin{problem}[\Oid{18}: Internal bounded L\"ob]
\label{prob:internal}
...
\begin{conjecture}[\Oid{19}: Bounded $\GL$, soundness form]
...
\begin{problem}[\Oid{20}: Parametric bounded Payor]
\label{prob:payor}
...
\textbf{The bounded argument needs internal arithmetic.} Critch \cite{critch2016} tracked symbol counts through the parametric fixed-point proof and its cooperation application, with budgets and thresholds left asymptotic. Under the conventions of this file, the repaired Theorem~\ref{thm:pbl} records the internal comparisons that proof consumes; the counterexamples and repaired statements are proved in full in the companion note \cite{sol2026}. The unbounded modal-agent theory is Bar\'asz, Christiano, Fallenstein, Herreshoff, LaVictoire, and Yudkowsky \cite{barasz2014}. Critch, Dennis, and Russell \cite{cdr2022} apply the parametric method to institution design and explain how to front-load the cooperation proof; Critch also asks for a Quinean proof of L\"ob's theorem \cite{critch2022quine}, whose constants could differ. The Payor lemma is recorded in Critch's post \cite{critchpayor2023}. On the mechanization side, $\GL$ has been formalized in HOL Light \cite{maggesi2023}; Duclaux et al.\ \cite{duclaux2026} formalize proof-based open-source games in Lean. Their workshop version axiomatized the bounded semantics and proof oracle; the library's main branch (merged July 27, 2026) removes every project axiom and proves bounded L\"ob inside its own proof calculus, with proof cost measured by transcript length and an explicit headroom constant, so that its outcome theorems depend only on Lean's standard axioms. It works in a bespoke calculus rather than a concrete arithmetic checker, and it does not address the counterexample of \cite{sol2026}. These are starting points for Starter~\ref{start:machine}, not substitutes for the certificate demanded here.
...
\begin{problem}[\Oid{21}: Robustness]
\label{prob:robust}
...
\begin{question}[\Oid{22}: Toward bounded Solovay]
\label{q:solovay}
...
\bibitem{duclaux2026}
C.~Duclaux, R.~Formenti, P.~Cobben, B.~Sch\"olkopf, and Z.~Jin,
\emph{Proving your way to cooperation: formalizing proof-based open source game theory in Lean},
3rd AI for Math Workshop at ICML, 2026.
\url{https://github.com/ColombanD/open-source-game-theory/tree/ai4math-icml-camera-ready}; main branch (July 27, 2026, project axioms removed): \url{https://github.com/ColombanD/open-source-game-theory}.
...
\bibitem{sol2026}
GPT 5.6 Sol,
\emph{A refutation and a repair of Critch's PBL theorem, with an application to program cooperation},
research note MAIS-P2, July 2026.
\url{https://github.com/lionellevine/MAIS/tree/main/papers/P2}
