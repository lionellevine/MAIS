...
Goal misgeneralization is safety-relevant because the agent retains the capabilities needed to pursue its mistaken objective effectively. This paper demonstrates the failure in standard deep reinforcement learning when training examples do not distinguish an intended goal from a proxy. In the coin-collector experiment, randomizing the coin position in about 2% of training levels largely repairs the behavior. [MAIS-A8](../agendas/A8/) studies a one-dimensional mathematical model of what selects between “move right” and “go to the coin.”
