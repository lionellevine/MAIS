...
A learning system can fail out of distribution in two ways, and only one of them scales with capability. Benign incompetence gets no worse as systems get stronger; goal misgeneralization does — an agent that keeps its skills while swapping its objective becomes more effective at pursuing the wrong thing as it improves, which is why competent-but-misdirected is the failure mode alignment actually fears. This paper shows the phenomenon is not hypothetical: it arises reliably from standard deep RL whenever the training distribution cannot distinguish the intended goal from a proxy, and the choice between them is made by the inductive biases of the training process, not by anything in the data. That turns a central alignment worry — do a system's goals generalize the way its capabilities do? — into a concrete question about learning dynamics, with two calibration facts any predictive theory must reproduce: the failure is reliable at zero training diversity, and about 2% disambiguating data largely repairs it. The mathematics of what selects the proxy, starting from this paper's coin-collector reduced to a one-dimensional coin line, is the subject of [MAIS-A8](../agendas/A8/).
