...
*Open problem MAIS-O57 · posed in [MAIS-A5](../agendas/A5/MAIS-A5.pdf) as [Problem 5.7](../agendas/A5/MAIS-A5.tex#L296) · Status: open.*
...
In words: at some decay strength and initialization scale, training charges at least two distinct representation sets with probability bounded away from zero for all late times. The seed data suggest the candidates $S = \{\mathrm{sgn}, \mathrm{std}\}$ and $S' = S \cup \{\text{a 5-dimensional}\}$. The static half of non-identifiability is known in a neighboring regime: for quadratic activation and squared loss on abelian tasks, Tian constructs abundant inequivalent zero-loss circuits. The dynamical half — that gradient flow from Gaussian initialization actually charges two key sets — is proved for no group and no architecture. For the surrounding definitions and results, see [MAIS-A5](../agendas/A5/MAIS-A5.pdf).
