...
$$\mathrm{vol}\bigl\{ w \in B_\delta(w^\ast) : |L(w) - L(w^\ast)| < \varepsilon \bigr\} \asymp \varepsilon^{\lambda(w^\ast)} \bigl( \log \tfrac1\varepsilon \bigr)^{m(w^\ast)-1},$$

so a small $\lambda$ means near-level parameters are plentiful; at a local minimum, $\lambda$ is Watanabe's real log canonical threshold. On the optimal set $W_0 = L^{-1}(0)$, the level sets $S_c = \{ w \in W_0 : \lambda(w) = c \}$ partition $W_0$; the agenda calls this the $\lambda$-stratification, and the name is aspirational. The question is closely related to Conjecture 5 of Lin (*J. Algebraic Stat.* 2017).
