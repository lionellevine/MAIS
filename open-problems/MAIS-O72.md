...
so a small $\lambda$ means near-level parameters are plentiful; at a local minimum, $\lambda$ is Watanabe's real log canonical threshold. The asymptotic exists, with $\lambda$ rational, by resolution of singularities: the zeta function $\int_{B_\delta(w^\ast)} |L(w) - L(w^\ast)|^{z} \, dw$ extends meromorphically to $\mathbb{C}$ with poles on finitely many arithmetic progressions of negative rationals (Arnold–Gusein-Zade–Varchenko), and $\lambda(w^\ast)$ is minus the largest pole, $m(w^\ast)$ its order. On the optimal set $W_0 = L^{-1}(0)$, the level sets $S_c = \lbrace  w \in W_0 : \lambda(w) = c \rbrace $ partition $W_0$; the agenda calls this the $\lambda$-stratification, and the name is aspirational. The question is closely related to Conjecture 5 of Lin (*J. Algebraic Stat.* 2017), which asserts that for a real-analytic function on a compact semianalytic set, the connected components of the level sets of the pair (threshold, multiplicity) form a stratification: pairwise disjoint locally closed pieces satisfying a frontier condition. Parts (a) and (b) below add finiteness of the value set and subanalyticity of the pieces.
...
Part of the frontier picture is now known: Lehalleur and Rimányi ([arXiv:2411.19920](https://arxiv.org/abs/2411.19920), Proposition 8.4(ii)) prove $\lambda$ is lower semicontinuous, so $\lbrace  w \in W_0 : \lambda(w) \le c \rbrace $ is closed in $W_0$. Finiteness of the image, subanalyticity of the pieces, and the multiplicity-refined version remain open. The real case has behavior absent from its complexification, including parity effects: in the main case of the Aoyagi–Watanabe formula for reduced rank regression, $\lambda$ gains an extra $\tfrac18$ and the multiplicity jumps from $1$ to $2$ precisely when the sum of the input dimension, output dimension, hidden width, and true rank is odd. For the surrounding program, see [MAIS-A7](../agendas/A7/).

## References

- S. Lin, *Ideal-theoretic strategies for asymptotic approximation of marginal likelihood integrals*, J. Algebraic Stat. **8** (2017), 22–55.
- S. P. Lehalleur, R. Rimányi, *Geometry of fibers of the multiplication map of deep linear neural networks*, 2024. [arXiv:2411.19920](https://arxiv.org/abs/2411.19920)
