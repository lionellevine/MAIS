...
so a small $\lambda$ means near-level parameters are plentiful; at a local minimum, $\lambda$ is Watanabe's real log canonical threshold [W09]. The asymptotic exists, with $\lambda$ rational, by resolution of singularities: the zeta function $\int_{B_\delta(w^\ast)} |L(w) - L(w^\ast)|^{z} \, dw$ extends meromorphically to $\mathbb{C}$ with poles on finitely many arithmetic progressions of negative rationals (Arnold–Gusein-Zade–Varchenko), and $\lambda(w^\ast)$ is minus the largest pole, $m(w^\ast)$ its order. On the optimal set $W_0 = L^{-1}(0)$, the level sets $S_c = \lbrace  w \in W_0 : \lambda(w) = c \rbrace $ partition $W_0$; the agenda calls this the $\lambda$-stratification, and the name is aspirational. The question is closely related to Conjecture 5 of Lin [L17], which asserts that for a real-analytic function on a compact semianalytic set, the connected components of the level sets of the pair (threshold, multiplicity) form a stratification: pairwise disjoint locally closed pieces satisfying a frontier condition. Parts (a) and (b) below add finiteness of the value set and subanalyticity of the pieces.
