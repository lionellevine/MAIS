...
Neither hypothesis in (a) can be dropped: with positive probability a neuron is inactive at initialization, and then its gradient is pure weight decay, so it shrinks radially and stays inactive, and impure, forever; and at $\delta \ge \tfrac12$ one neuron can be pure for both classes at once. For maximum margin in the $\ell_{2,3}$ norm ($\Vert \theta\Vert _{2,3} = (\sum_i \Vert \omega_i\Vert _2^3)^{1/3}$, where $\omega_i$ concatenates the weights of neuron $i$), Morwani, Edelman, Oncescu, Zhao, and Kakade [[MEOZK24]](https://arxiv.org/abs/2311.07568) prove every nonzero neuron pure with both representations present — a different regularizer, so a motivation rather than a prediction here. For the quadratic activation without weight decay, He, Wang, Zhang, Chen, and Yang [[HWZCY26]](https://arxiv.org/abs/2606.02993) prove alignment for projected flow under a Taylor-surrogate risk; for the exact-loss, rectifier, and weight-decay regimes nothing is proved; see [MAIS-A5](../agendas/A5/).
