...
1. Determine $\delta^*(m,k,S)=\inf\{\delta\in[0,1] : P'_{m,k,S}(\delta)<P'_{m,k,S}(0)\}$, with the convention $\delta^*=1$ when the set is empty. Is $\delta^*=0$ for all $S$ close to $1$ (any polysemanticity budget helps), or is there a threshold?
