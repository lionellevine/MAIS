...
Fix a proof system $S$ that is *efficient* in Critch's sense, meaning three things: $S$ is a consistent, recursively axiomatized extension of Peano arithmetic; the numeral for $k$ has length $O(\log k)$ (binary numerals rather than $SS\cdots S0$); and proofs may introduce abbreviations, so that a long expression used many times is paid for once. Write $S\vdash_{\le k}\varphi$ for "$\varphi$ has an $S$-proof of at most $k$ symbols," $\ell_S(\varphi)$ for the least length in symbols of an $S$-proof of $\varphi$, $|\varphi|$ for the length of the formula $\varphi$, and $\Box P$ for the arithmetized statement "$P$ is provable in $S$." The concrete default $\mathsf{PA}_{\mathrm{bin}}$ is Peano arithmetic in Enderton's Hilbert calculus with binary numerals and a priced abbreviation mechanism: an abbreviation line defines a fresh name to stand for a fixed string, possibly using earlier abbreviations; every character of the name and its definition is charged, and proof length counts the proof file as written, so later uses cost only the name. (The conventions matter: the overhead defined below depends on the calculus, the numerals, the abbreviation discipline, and the coding; other concrete systems pose parallel questions, with different answers.) The agenda's Definition 3.1 sets
