...
1. For $(m,n)=(2,1)$ with importance weights $(1,I)$, on convergent trajectories write the limiting row as $(w_1,w_2)$. Determine the probabilities of the four disjoint events
   - $D_2=\{w_2=0,\ w_1\ne0\}$ — drop feature 2,
   - $A_2=\{w_1=0,\ w_2\ne0\}$ — dedicate the dimension to feature 2,
   - $S_{\pm}=\{w_1w_2<0\}$ — store the pair antipodally,
...
3. Fix instead $(m,k,S,\sigma)$ with $k\ge m$ and run the selected Clarke trajectory for $L'$ on $\theta=(W,\beta,V,c)$, initialized with $W_{ij},V_{ij}\sim N(0,\sigma^2)$ independently and both bias vectors zero. Determine the law of $M(\theta_\infty)$ on $[1/m,1]\cup\{\dagger\}$, interpreting $M(\theta_\infty)=\dagger$ on nonconvergent trajectories.
