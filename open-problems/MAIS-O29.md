...
The framework is a binary causal influence diagram, the setting in which Richens and Everitt [[RE24]](https://arxiv.org/abs/2402.10877) proved that robustly near-optimal behavior generically determines the environment's causal model. A **model** $M=(G,\theta)$ is a directed acyclic graph $G$ on binary chance variables together with conditional probability tables $\theta$: nature runs the network, the agent sees an observation subset, picks a binary action $d$, and is scored by a known utility $u(d,z)$ depending on $d$ and the values $z$ of a utility-parent set $\mathbf{Z}$. The analyst knows this fixed data — the **skeleton** $\mathsf{s}$ — and must recover $G$ and $\theta$. The **margin class** $\mathcal{M}(\mathsf{s},\lambda)$ consists of the models free of degeneracy at scale $\lambda$: table entries, utility gaps, and edge strengths all bounded away from zero by $\lambda$, in the sense of conditions (M1)–(M6) of [MAIS-A2](../agendas/A2/).
