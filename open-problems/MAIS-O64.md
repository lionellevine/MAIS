...
The model is the quadratic modular-addition network of [MAIS-A6](../agendas/A6/MAIS-A6.pdf): on input $(a,b) \in (\mathbb{Z}/p\mathbb{Z})^2$ it computes $f_w(a,b) = \sum_{j=1}^H V_j (u_j'(a)+u_j''(b))^2 \in \mathbb{R}^p$, the population loss $K$ is half the mean squared error against the one-hot target $\delta_{a+b}$, and $W_0(p,H)$ is its zero set. Write $\lambda(w)$ for the local learning coefficient of $K$ at $w$ (the volume-growth exponent of $\{K \le \varepsilon\}$ near $w$), $R_p = 1 + \|w^F\|$ for the norm scale of the width-$(2p-1)$ Fourier fit $w^F$, and $\lambda_{\min}(p,H;R) = \min\{\lambda(w) : w \in W_0(p,H),\ \|w\| \le R\}$.
