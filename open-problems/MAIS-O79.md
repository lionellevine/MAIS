...
The test model is the two-layer linear network $L(A,B) = \tfrac12 \Vert  BA - \Phi \Vert _F^2$ with target $\Phi$ of rank $r$ and distinct singular values $s_1 > \dots > s_r > 0$; the agenda calls it the deep linear network. Its critical points are classified by Zhou and Liang [[ZL17]](https://arxiv.org/abs/1710.11205): $C_k$ is the set of weights at which the network has learned exactly the top $k$ modes of $\Phi$, a saddle set for $k < r$ and the optimal set for $k = r$. Gradient flow from the **aligned initialization** at scale $u_0$, which seeds every mode at strength $u_0$, decouples across modes (Saxe, McClelland, and Ganguli [[SMG14]](../references/SMG14.md)): as $u_0 \to 0$ the flow passes along the saddle chain $C_0, C_1, \dots, C_r$ in order, leaving $C_{k-1}$ at an explicit time $t_k$ of order $\log(1/u_0)$. Exact formulas are in [MAIS-A7](../agendas/A7/), Section 2.4.
