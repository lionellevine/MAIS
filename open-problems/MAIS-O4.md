...
In words: how small can the task loss be among weight matrices whose worst-case interference is at most $c$ — the exact price of each increment of legibility. Empirically the optimizer jumps between regular-polytope configurations (antipodal pairs, triangles, pentagons) as sparsity varies, as Elhage et al. [[EHOS+22]](https://arxiv.org/abs/2209.10652) observed, which suggests the frontier is piecewise smooth with kinks at changes of optimal geometry; part (2) asks for that picture as a theorem. Even the comparison point in part (3), the unconstrained value $P_{m,n,S}(1)$, is not known in closed form for general $(m,n,S)$; the closest result, by Cowsik, Dolev, and Infanger [[CDI24]](https://arxiv.org/abs/2410.12101), is a near-optimal high-sparsity scaling from a permutation-symmetric large-$m$ ansatz. The survey's stated first case — whether penalizing the *average* interference lowers the *worst-case* coherence of the minimizer — is [MAIS-O44](MAIS-O44.md). For the proved anchors, a subadditivity argument giving a thermodynamic limit of the frontier, and the surrounding program, see [MAIS-A4](../agendas/A4/).
