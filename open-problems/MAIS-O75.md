...
The dynamics is Langevin at temperature $\varepsilon$, $dX_t = -\nabla L(X_t)\ dt + \sqrt{2\varepsilon}\ dB_t$; on the torus this process is ergodic, with invariant Gibbs measure $\pi_\varepsilon(dw) \propto e^{-L(w)/\varepsilon}\  dw$, and the spectral gap in part (b) is that of the generator in $L^2(\pi_\varepsilon)$. Flatness is measured by the local learning coefficient $\lambda$ and its multiplicity $m$: the volume of nearby parameters at loss within $\varepsilon$ of the point scales as $\varepsilon^{\lambda} (\log \tfrac{1}{\varepsilon})^{m-1}$, so smaller $\lambda$ means a wider channel of near-optimal parameters, and $m = 1$ means no logarithmic correction.
...
- S. Wojtowytsch, *Stochastic gradient descent with noise of machine learning type. Part II: continuous time analysis*, J. Nonlinear Sci. 33 (2023), Paper No. 45. [arXiv:2106.02588](https://arxiv.org/abs/2106.02588)
