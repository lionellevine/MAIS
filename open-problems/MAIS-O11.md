...
Part 1 asks for a sequence of sentences whose direct proofs are longer, by a fixed factor plus the trivial toll, than the proofs of their reflection instances: the first witness that the detour pays at all. In part 2, Conjecture 3.5 asserts the first alternative, the polynomial bound: there are constants $C, c$ with $F_{\mathsf{PA}_{\mathrm{bin}}}(k,n) \le C(k+n)^c$ for all $k, n$. The nearest proved lower bounds are Friedman's and Pudlák's [[P17]](https://arxiv.org/abs/1601.01487), for the one instance $P=\bot$ where the hypothesis is unprovable; for the speedup literature and why it falls short here, see [MAIS-A1](../agendas/A1/).
