...
Work in an efficient proof system $S$, in Critch's sense: a consistent, recursively axiomatized extension of Peano arithmetic whose numerals are binary (the numeral for $k$ has length $O(\log k)$) and whose proofs may introduce abbreviations — a line defining a fresh symbol to stand for a fixed string, possibly using earlier abbreviations, so that a long expression used many times is paid for once. Write $\ell_S(\varphi)$ for the least length of an $S$-proof of $\varphi$, counted in symbols (the size of the proof as a text file, not its number of lines), and $\Box P$ for arithmetized provability. The default $\mathsf{PA}_{\mathrm{bin}}$ is Peano arithmetic in a Hilbert calculus that the agenda fixes down to the abbreviation grammar and the Gödel coding, since proof lengths depend non-cosmetically on such conventions. One direction is cheap: appending to a shortest proof of $P$ one propositional axiom instance and one modus ponens gives a constant $C_1 = C_1(S)$ with $\ell_S(\Box P \to P) \le \ell_S(P) + C_1(|P|+1)$ for every provable $P$ (agenda, Lemma 3.3). So the reflection instance is never much harder than the sentence; the question is whether it can be genuinely easier. Parikh's speedup — theorems $A$ for which $\Box A$ has a short proof while every proof of $A$ is far longer — does not settle it, because a short proof of $\Box A \to A$ would combine with the short proof of $\Box A$ to give a short proof of $A$. The overhead $F_{\mathsf{PA}_{\mathrm{bin}}}(k,n)$ below is the maximum of $\ell_S(P)$ over sentences $P$ with $|P|\le n$ and $\ell_S(\Box P\to P)\le k$ (see [MAIS-O1](MAIS-O1.md)).
...
Part 1 asks for a sequence of sentences whose direct proofs are longer, by a fixed factor plus the trivial toll, than the proofs of their reflection instances: the first witness that the detour pays at all. In part 2, Conjecture 3.5 asserts the first alternative, the polynomial bound: there are constants $C, c$ with $F_{\mathsf{PA}_{\mathrm{bin}}}(k,n) \le C(k+n)^c$ for all $k, n$. The nearest proved lower bounds are Friedman's and Pudlák's, for the one instance $P=\bot$ where the hypothesis is unprovable; for the speedup literature and why it falls short here, see [MAIS-A1](../agendas/A1/).
...
- R. Parikh, *Existence and feasibility in arithmetic*, Journal of Symbolic Logic 36 (1971), no. 3, 494–508.
