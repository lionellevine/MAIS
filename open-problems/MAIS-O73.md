...
Two terms in the statement come from the potential-theoretic approach to metastability of Bovier, Eckhoff, Gayrard, and Klein [BEGK04]: the **gate** is the set where the infimum defining $h$ is attained (the passes at level $h$ that near-optimal crossings go through), and the **capacity** between the two wells is their potential-theoretic conductance. The mean crossing time is then the ratio of the starting well's Gibbs mass to the capacity, so the resolution data along $K_A$ enters through the mass and the gate data through the capacity.

When the gate is a single Morse saddle, the agenda conjectures $\alpha = \lambda_A - d/2$ and $q = m_A - 1$ ([MAIS-O74](MAIS-O74.md)): flatter wells are stickier, by precisely the learning coefficient's deficit from $d/2$. What is known stops short: Berglund–Gentz treat specific degenerate normal forms (surveyed in [[B13]](https://arxiv.org/abs/1106.5799)), Assal–Bony–Michel [[ABM25]](https://arxiv.org/abs/2202.02208) settle Morse–Bott wells, and Delande [[Del26]](https://arxiv.org/abs/2410.21899) treats isolated wells and gates of diagonal monomial type; none covers an arbitrary analytic singular well with exponents given by its resolution data. For a degenerate positive-dimensional gate, even the conjectural prefactor needs spectral data that no volume invariant sees. Full setting and references in [MAIS-A7](../agendas/A7/).
